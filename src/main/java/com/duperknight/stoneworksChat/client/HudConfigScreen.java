...
public class HudConfigScreen extends Screen {
    ...
    // Use standard texture identifiers (keep file under assets/modid/textures/tutorial/*.png)
    private static final Identifier ARROWS_TEX = Identifier.of(StoneworksChatClient.MOD_ID, "textures/tutorial/arrow_keys.png");
    private static final Identifier SHIFT_TEX = Identifier.of(StoneworksChatClient.MOD_ID, "textures/tutorial/shift_key.png");
    ...
    showTutorialPopup = StoneworksChatClient.showHudTutorial;
        ...
        try {
            ...
            if (StoneworksChatClient.hudAnchorX != null && StoneworksChatClient.hudAnchorY != null &&
                ...
                float leftXf; // left edge (float for symmetric rounding)
                ...
                switch (StoneworksChatClient.hudTextAlign) {
                    ...
                    case CENTER -> {
                        float mid = leftXf + scaledBgW / 2f;
                        this.currentX = centerAnchorX ? (int)Math.floor(mid + 0.0001f) : Math.round(mid);
                    }
                    case RIGHT_TO_LEFT -> {
                        float right = leftXf + scaledBgW;
                        this.currentX = centerAnchorX ? (int)Math.floor(right + 0.0001f) : Math.round(right);
                    }
                }
                boolean centerAnchorY = StoneworksChatClient.hudAnchorY == StoneworksChatClient.AnchorY.CENTER;
                this.currentY = centerAnchorY ? (int)Math.floor(topYf + 0.0001f) : Math.round(topYf);
            }
    ...
    doneBtn = addDrawableChild(ButtonWidget.builder(Text.translatable("gui.done"), b -> {
        ...
        // Decide anchor X based on proximity of left edge, center line, right edge
        int distLeft = leftX;
        int distCenter = Math.abs((leftX + boxW / 2) - (screenW / 2));
        int distRight = screenW - (leftX + boxW);
        StoneworksChatClient.hudAnchorX = (distCenter <= distLeft && distCenter <= distRight)
        ...
        switch (StoneworksChatClient.hudAnchorX) {
            ...
            case CENTER -> {
                float centerOffset = (leftX + boxW / 2f) - (screenW / 2f);
                StoneworksChatClient.hudOffsetX = Math.round(centerOffset);
            }
            case RIGHT -> StoneworksChatClient.hudOffsetX = Math.round(screenW - (leftX + boxW));
        }

        int topY = currentY;
        int distTop = topY;
        int distMid = Math.abs((topY + boxH / 2) - (screenH / 2));
        int distBot = screenH - (topY + boxH);
        StoneworksChatClient.hudAnchorY = (distMid <= distTop && distMid <= distBot)
            ? StoneworksChatClient.AnchorY.CENTER
            : (distTop <= distBot ? StoneworksChatClient.AnchorY.TOP : StoneworksChatClient.AnchorY.BOTTOM);
        switch (StoneworksChatClient.hudAnchorY) {
            case TOP -> StoneworksChatClient.hudOffsetY = Math.round(topY);
            case CENTER -> {
                float centerOffsetY = (topY + boxH / 2f) - (screenH / 2f);
                StoneworksChatClient.hudOffsetY = Math.round(centerOffsetY);
            }
    ...
    public void render(DrawContext drawContext, int mouseX, int mouseY, float delta) {
        ...
        drawContext.drawCenteredTextWithShadow(textRenderer, this.title, this.width / 2, 12, 0xFFFFFFFF);
        ...
        if (channelInfo != null) {
            ...
            if ((colorCode & 0xFF000000) == 0) colorCode |= 0xFF000000; // ensure opaque
            if (StoneworksChatClient.hudVisible) {
                drawContext.getMatrices().pushMatrix();
                drawContext.getMatrices().translate((float) textXLogical, (float) textY);
            ...
            if (resizing && activeHandle != null) {
                ...
                drawContext.drawCenteredTextWithShadow(textRenderer, Text.literal(scaleStr), labelX, labelY, 0xFFFFFFFF);
            }
        }
        drawContext.drawCenteredTextWithShadow(textRenderer, Text.translatable("screen.stoneworks_chat.hud_config.drag_help"), this.width / 2, this.height - 84, 0xFFAAAAAA);
        ...
        drawContext.drawCenteredTextWithShadow(textRenderer, vis, this.width / 2, this.height - 72, 0xFFCCCCCC);
        drawContext.getMatrices().popMatrix();
    }
    ...
    private void drawGuides(DrawContext ctx, int boxW, int boxH) {
        ...
        if (Math.abs(boxCenterX - screenCenterX) <= tolerance) {
            ctx.fill(screenCenterX, 0, screenCenterX + 1, screenH, centerColor); // full height now
        }
    ...
    private static int clamp(int v, int min, int max) { return v < min ? min : Math.min(v, max); }
    ...
    private boolean handleDoubleClick(double mouseX, double mouseY) {
        ...
        if (now - lastHandleClickTime <= DOUBLE_CLICK_MS) { lastHandleClickTime = 0L; return true; }
        lastHandleClickTime = now; return false;
    }
    ...
    private boolean hitPreview(double mouseX, double mouseY) {
        ...
        return mouseX >= leftX && mouseX <= leftX + scaledW && mouseY >= currentY && mouseY <= currentY + scaledH;
    }
    ...
    private void hideTutorialButtons() { if (tutorialOkBtn != null) tutorialOkBtn.visible = false; if (tutorialDontShowBtn != null) tutorialDontShowBtn.visible = false; }

    private Integer arrowsWCache, arrowsHCache, shiftWCache, shiftHCache;
    private void ensureTextureSizes() {
        if (arrowsWCache != null && shiftWCache != null) return;
        var mc = MinecraftClient.getInstance();
        var rm = mc.getResourceManager();
        try {
            rm.getResource(ARROWS_TEX).ifPresent(r -> { try (var is = r.getInputStream()) { var img = NativeImage.read(is); arrowsWCache = img.getWidth(); arrowsHCache = img.getHeight(); } catch (Exception ignored) {} });
            rm.getResource(SHIFT_TEX).ifPresent(r -> { try (var is = r.getInputStream()) { var img = NativeImage.read(is); shiftWCache = img.getWidth(); shiftHCache = img.getHeight(); } catch (Exception ignored) {} });
        } catch (Exception ignored) {}
    ...
    private void renderTutorialPanel(DrawContext ctx) {
        ...
        ctx.drawCenteredTextWithShadow(textRenderer, Text.translatable("screen.stoneworks_chat.hud_config.tutorial.title"), x + popupW / 2, y + 6, 0xFFFFFFFF);
        int contentY = y + 22;
        int blockStartX = x + (popupW - contentBlockW) / 2;

        int arrowsImgX = blockStartX;
        int arrowsImgY = contentY + (Math.max(arrowsH, textRenderer.fontHeight) - arrowsH) / 2;
        drawIcon(ctx, ARROWS_TEX, arrowsImgX, arrowsImgY, arrowsW, arrowsH);
        int arrowsTextX = arrowsImgX + arrowsW + spaceBetween;
        int arrowsTextY = contentY + (Math.max(arrowsH, textRenderer.fontHeight) - textRenderer.fontHeight) / 2;
        ctx.drawTextWithShadow(textRenderer, Text.literal(arrowsLine), arrowsTextX, arrowsTextY, 0xFFDDDDDD);

        int secondY = contentY + Math.max(arrowsH, textRenderer.fontHeight) + lineSpacing;
        int shiftImgX = blockStartX;
        int shiftImgY = secondY + (Math.max(shiftH, textRenderer.fontHeight) - shiftH) / 2;
        drawIcon(ctx, SHIFT_TEX, shiftImgX, shiftImgY, shiftW, shiftH);
        int shiftTextX = shiftImgX + shiftW + spaceBetween;
        int shiftTextYPos = secondY + (Math.max(shiftH, textRenderer.fontHeight) - textRenderer.fontHeight) / 2;
        ctx.drawTextWithShadow(textRenderer, Text.literal(shiftLine), shiftTextX, shiftTextYPos, 0xFFDDDDDD);
    ...
    private void drawIcon(DrawContext ctx, Identifier tex, int x, int y, int w, int h) {
        ctx.drawTexture(RenderPipelines.GUI_TEXTURED, tex, x, y, 0f, 0f, w, h, w, h);
    }

    @Override
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        if (showTutorialPopup) return super.mouseClicked(mouseX, mouseY, button);
        if (resizing && button != 0) return super.mouseClicked(mouseX, mouseY, button);
        if (button == 1) {
            if (hitPreview(mouseX, mouseY)) {
                StoneworksChatClient.hudVisible = !StoneworksChatClient.hudVisible; ChatConfig.save(); return true;
            }
        }
        if (button == 0) {
            if (handleDoubleClick(mouseX, mouseY)) { workingScale = Math.round(1.0f / SCALE_SNAP_INCREMENT) * SCALE_SNAP_INCREMENT; return true; }
            if (hoveringTL || hoveringTR || hoveringBL || hoveringBR) {
                resizing = true; startScale = workingScale;
                ...
                double dx = mouseX - resizeAnchorX; double dy = mouseY - resizeAnchorY;
    ...
    public boolean mouseDragged(double mouseX, double mouseY, int button, double dx, double dy) {
        ...
        if (resizing && button == 0) {
            ...
            if (newScale != workingScale) {
                ...
                int anchorLeftX; int anchorTopY;
                switch (activeHandle) {
                    case TOP_LEFT -> { anchorLeftX = resizeAnchorX - newW; anchorTopY = resizeAnchorY - newH; }
                    case TOP_RIGHT -> { anchorLeftX = resizeAnchorX; anchorTopY = resizeAnchorY - newH; }
                    case BOTTOM_LEFT -> { anchorLeftX = resizeAnchorX - newW; anchorTopY = resizeAnchorY; }
                    case BOTTOM_RIGHT -> { anchorLeftX = resizeAnchorX; anchorTopY = resizeAnchorY; }
                    default -> { anchorLeftX = 0; anchorTopY = 0; }
                }
                if (rtl) currentX = anchorLeftX + newW; else if (centerAlign) currentX = anchorLeftX + newW / 2; else currentX = anchorLeftX;
                currentY = anchorTopY;
                if (rtl) currentX = clamp(currentX, newW, this.width); else if (centerAlign) currentX = clamp(currentX, newW / 2, this.width - newW / 2); else currentX = clamp(currentX, 0, this.width - newW);
        ...
        if (dragging && button == 0) {
            ...
            switch (currentAlign) {
                case RIGHT_TO_LEFT -> { currentX = leftX + scaledW; currentX = clamp(currentX, scaledW, this.width); }
                case CENTER -> { currentX = leftX + scaledW / 2; currentX = clamp(currentX, scaledW / 2, this.width - scaledW / 2); }
                default -> { currentX = leftX; currentX = clamp(currentX, 0, this.width - scaledW); }
            }
    ...
    public boolean mouseReleased(double mouseX, double mouseY, int button) {
        ...
        if (resizing && button == 0) { resizing = false; activeHandle = null; return true; }
    ...
    public boolean keyPressed(int keyCode, int scanCode, int modifiers) {
        if (showTutorialPopup) {
            if (keyCode == InputUtil.GLFW_KEY_ESCAPE) { showTutorialPopup = false; hideTutorialButtons(); return true; }
            return super.keyPressed(keyCode, scanCode, modifiers);
        }
        int step = hasShiftDown() ? 5 : 1;
        boolean handled = false;
        if (keyCode == InputUtil.GLFW_KEY_LEFT) { currentX -= step; handled = true; } else if (keyCode == InputUtil.GLFW_KEY_RIGHT) { currentX += step; handled = true; }
        if (keyCode == InputUtil.GLFW_KEY_UP) { currentY -= step; handled = true; } else if (keyCode == InputUtil.GLFW_KEY_DOWN) { currentY += step; handled = true; }
        if (handled) {
            int scaledW = Math.round(previewW * workingScale);
            int scaledH = Math.round(previewH * workingScale);
            switch (currentAlign) {
                case RIGHT_TO_LEFT -> currentX = clamp(currentX, scaledW, this.width);
                case CENTER -> currentX = clamp(currentX, scaledW / 2, this.width - scaledW / 2);
                default -> currentX = clamp(currentX, 0, this.width - scaledW);
            }
            currentY = clamp(currentY, 0, this.height - scaledH);
            return true;
        }
